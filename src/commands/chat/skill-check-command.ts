...
import { FoundryActor, foundryCpr, FoundryModel } from '../../services/foundry-service.js';
...
async function getFilteredActors(
    actors: Map<string, FoundryActor>,
    match: string,
    allTypes?: boolean
    ...
    const result = allTypes
        ? filteredActors
        : filteredActors.filter(actor => actor.isPlayerCharacter);
    return result.map(foundryToChoice);
}

async function getFilteredSkills(
    character: FoundryActor,
    match: string
): Promise<ApplicationCommandOptionChoiceData[]> {
...
export class SkillCheckCommand implements Command {
    ...
    private async getDieImage(value: number, useSpecials?: boolean): Promise<string | undefined> {
        ...
        return await foundryCpr.getImageUrl(
            `systems/cyberpunk-red-core/icons/dice/red/d10_${dieName}.svg`
        );
    }
}
